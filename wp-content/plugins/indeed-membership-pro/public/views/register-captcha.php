<?php if ( $type !== false && $type == 'v3' ):?>
    ...
    <?php wp_enqueue_script( 'ihc-login-captcha-google', 'https://www.google.com/recaptcha/api.js?render=' . $key, ['jquery'], '13.0' );?>
    <?php wp_enqueue_script( 'ihc-login-captcha', IHC_URL . 'assets/js/captcha.js', ['jquery'], '13.0' );?>
<?php else :?>
    <div class="g-recaptcha-wrapper <?php if(!empty($class)){
      echo esc_attr($class);
    }?>">
        <div class="g-recaptcha" data-sitekey="<?php echo esc_attr($key);?>"  ></div>
        <?php wp_enqueue_script( 'ihc-login-captcha-google', 'https://www.google.com/recaptcha/api.js?hl=' . $langCode, ['jquery'], '13.0');?>
    </div>
<?php endif;?>
