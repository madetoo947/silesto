<div class="ihc-subtab-menu">
    <?php ?>
    <a class="ihc-subtab-menu-item  <?php echo ( isset($_REQUEST['ihc-new-user']) && $_REQUEST['ihc-new-user']  == 'true') ? 'ihc-subtab-selected' : '';?>"
        href="<?php echo esc_url($url . '&tab=users&ihc-new-user=true');?>"><?php esc_html_e('Add New Member', 'ihc');?></a>
    <a class="ihc-subtab-menu-item  <?php echo ( !isset($_REQUEST['ihc-new-user'])) ? 'ihc-subtab-selected' : '';?>"
        href="<?php echo esc_url($url . '&tab=' . $tab );?>"><?php esc_html_e('Manage Members', 'ihc');?></a>

    <div class="ihc-clear"></div>
</div>
<?php
wp_enqueue_script( 'ihcAdminSendEmail', IHC_URL . 'admin/assets/js/ihcAdminSendEmail.js', ['jquery'], '13.1' );
wp_enqueue_script( 'ihcSearchUsers', IHC_URL . 'admin/assets/js/search_users.js', ['jquery'], '13.0' );
    ...
    <div class="inside">
        <div class="ihc-admin-edit-user">
            <div class="ihc-admin-user-form-wrapper">
                ...
                <p><?php esc_html_e('Manage what fields are available for Admin setup from "Showcases->Register Form->Custom Fields" section ', 'ihc');?>
                </p>
            </div>
...
<div class="iump-wrapper">
    <div id="col-right" class="ihc-admin-listing-users">
        ...
        <div class="ihc-special-buttons-users">
            <div class="ihc-special-button ihc-list-user-make-csv" id="ihc_make_user_csv_file" data-filters=""
                data-get_variables=""><i class="fa-ihc fa-export-csv"></i><?php esc_html_e( 'Export CSV', 'ihc' );?>
            </div>
            <div class="ihc-hidden-download-link"><a href=""
                    target="_blank"><?php esc_html_e( "Click on this if download doesn't start automatically in 20 seconds!", 'ihc');?></a>
            </div>
            <div class="ihc-clear"></div>
        </div>
        ...
        <div class="iump-datatable-filters-wrapper">
            <br />
            <input type="text" value="" placeholder="<?php esc_html_e('Search Members', 'ihc');?>"
                class="iump-js-search-phrase ump-max-width-300">
            ...
            <div class="iump-datatable-multiselect-wrapp">
                ...
                <?php
									// Сортировка ассоциативного массива по 'label' без потери ключей
									uasort($levels_arr, function($a, $b) {
											return strcmp($a['label'], $b['label']);
									});
								?>
                <select name="memberships[]" class="iump-datatable-filter-users-memberships" multiple
                    data-placeholder="<?php esc_html_e("Choose Memberships", 'ihc');?>">
                    <?php if ( $levels_arr ):?>
            ...
            <div class="iump-datatable-multiselect-wrapp">
                ...
                <?php
										...
										?>
                <?php endif;?>
                <select name="membership_status[]" class="iump-datatable-filter-users-membership-status" multiple
                    data-placeholder="<?php esc_html_e("Memberships status", 'ihc');?>">
            ...
            <?php
						...
						?>
            <div class="iump-datatable-multiselect-wrapp">
                <select name="roles[]" class="iump-datatable-filter-users-roles" multiple
                    data-placeholder="<?php esc_html_e("WordPress Roles", 'ihc');?>">
            ...
            <div class="iump-datatable-multiselect-wrapp">
                <?php
								...
								?>
                <select name="administrator_requests" class="iump-datatable-filter-users-extra_conditions" multiple
                    data-placeholder="<?php esc_html_e("Manager Requests", 'ihc');?>">
            ...
            <div class="iump-datatable-date-input-wrapp">
                <button type="button"
                    class="button button-primary button-small iump-js-admin-users-submit-filters-bttn iump-filters-bttn"><?php esc_html_e('Filter', 'ihc');?></button>
            </div>
...
$columns = [
			  ...
			  [
					'data'        => 'silesto_balance',
					'orderable'   => false,
					'sortable'    => false,
					'className'   => 'ihc-users-table-col-balance',
				],
				[
						'data'			=> 'user_sites',
						'orderable' => false,
			...
			wp_enqueue_script( 'ihcmultiselectfunctions', IHC_URL . 'admin/assets/js/jquery.multiselect.js', ['jquery'], '13.0' );
			...
			// js
			wp_enqueue_script( 'ihcdatabse', IHC_URL . 'admin/assets/js/datatables/datatables.min.js', ['jquery'], '13.0' );
			wp_enqueue_script( 'ihcdatabse-buttons', IHC_URL . 'admin/assets/js/datatables/dataTables.buttons.min.js', ['jquery'], '13.0' );
			wp_enqueue_script( 'ihcdatabse-colvis', IHC_URL . 'admin/assets/js/datatables/buttons.colVis.min.js', ['jquery'], '13.0' );
			// iump datatable functions
			wp_register_script( 'ihc-table', IHC_URL . 'admin/assets/js/table.js', ['jquery'], '13.0' );
...
?>
        ...
        <table id="iump-dashboard-table" class="display iump-dashboard-table ihc-display-none iump-js-orders-table">
            <thead>
                <tr>
                    <th class=""><input type="checkbox" class="iump-js-select-all-checkboxes"
                            data-target="iump-dashboard-table" /></th>
                    <th class="iump-dashboard-table-head-col iump-dashboard-table-col-sorting">
                        <?php esc_html_e('ID', 'ihc');?></th>
                    <th class="iump-dashboard-table-head-col iump-dashboard-table-col-sorting">
                        <?php esc_html_e('Member Name', 'ihc');?></th>
                    <th class="iump-dashboard-table-head-col iump-dashboard-table-col-sorting">
                        <?php esc_html_e('Email Address', 'ihc');?></th>
                    <th class="iump-dashboard-table-head-col iump-dashboard-table-col-sorting">
                        <?php esc_html_e('Membership Plans', 'ihc');?></th>
                    <th class="iump-dashboard-table-head-col iump-dashboard-table-col-sorting">
                        <?php esc_html_e('Total Spend', 'ihc');?></th>
                    <th class="ihc-users-list-custom-col"><?php esc_html_e('Баланс ($)', 'ihc');?></th>
                    <?php do_action('ump_action_admin_list_user_column_name_after_total_spend');?>
                    <?php if ( $magic_feat_user_sites ):?>
                    <th class="iump-dashboard-table-head-col iump-dashboard-table-col-sorting">
                        <?php esc_html_e('Sites', 'ihc');?></th>
                    <?php endif;?>
                    <th class="iump-dashboard-table-head-col iump-dashboard-table-col-sorting">
                        <?php esc_html_e('WordPress Role', 'ihc');?></th>
                    <th class="iump-dashboard-table-head-col iump-dashboard-table-col-sorting">
                        <?php esc_html_e('Email Status', 'ihc');?></th>
                    <th class="iump-dashboard-table-head-col iump-dashboard-table-col-sorting">
                        <?php esc_html_e('Member Since', 'ihc');?></th>
                    <th class="iump-dashboard-table-head-col iump-dashboard-table-col-sorting">
                        <?php esc_html_e('Details', 'ihc');?></th>
            </thead>
        </table>
        ...
        <div class="iump-datatable-actions-wrapp-copy ihc-display-none">
            ...
            <input type="submit" name="iump-datatable-submit" value="<?php esc_html_e('Apply', 'ihc');?>"
                class="button button-primary button-small iump-js-members-apply-bttn" />
        </div>
...
jQuery(document).ready(function($) {
    // Используем делегирование событий для динамически загружаемой таблицы
    $('#iump-dashboard-table').on('blur', '.silesto-balance-input', function() {
        var $input = $(this);
        // Если значение не изменилось, ничего не делаем
        if ($input.data('initial-value') == $input.val()) return;

        var $wrapper = $input.closest('.silesto-balance-wrapper');
        var $spinner = $wrapper.find('.silesto-balance-spinner');
        var $status = $wrapper.find('.silesto-balance-status');

        var uid = $input.data('uid');
        var new_balance = $input.val();
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'silesto_update_user_balance_ajax', // Наш AJAX-обработчик
                user_id: uid,
                balance: new_balance,
                nonce: '<?php echo wp_create_nonce("silesto_balance_nonce"); ?>' // Защита
            },
            success: function(response) {
                $spinner.hide();
                if (response.success) {
                    $status.html('&#x2714;').css('color', 'green'); // Галочка
                    $input.data('initial-value',
                    new_balance); // Обновляем начальное значение
                } else {
                    $status.html('&#x2718;').css('color', 'red'); // Крестик
                }
    ...
    // Запоминаем начальное значение при фокусе на поле
    $('#iump-dashboard-table').on('focus', '.silesto-balance-input', function() {
