...
function ihc_get_redirect_links_as_arr_for_select()
{
	...
	if (ihc_is_magic_feat_active('individual_page')){
		$return['#individual_page#'] =  esc_html__('Отдельная страница (из расширений)', 'ihc');
	}
	return $return;
}
		...
		if ($paid_levels){
			...
			if ($err_msg){
				return '<div class="ihc-error-global-dashboard-message">' .
							'<strong>' . esc_html__( 'Предупреждение платежного сервиса:', 'ihc') . '</strong> '
				. esc_html__("Похоже, вы ничего не настроили ", 'ihc') . '<a href="' . admin_url( 'admin.php?page=ihc_manage&tab=payment_settings&subtab=stripe_connect' ) . '">' . esc_html__( 'платежные услуги', 'ihc' ) . "</a>" . esc_html__(" для вашего членского сайта. Пожалуйста, настройте свой ", 'ihc') . '<a href="' . admin_url( 'admin.php?page=ihc_manage&tab=payment_settings&subtab=stripe_connect' ) . '">' . esc_html__( "настройки оплаты", 'ihc' ) . "</a> " . esc_html__("чтобы включить членские платежи", 'ihc') . '</div>';
			}else{
				$suggestion_msg = TRUE;
				foreach ($payments_gateways as $payment_gateway => $label ){
				...
				if($suggestion_msg && empty( $hideSuggestion ) ){
					return '<div class="ihc-error-global-dashboard-message">' .
								...
								'<strong>' . esc_html__( 'Предупреждение платежного сервиса:', 'ihc') . '</strong> '
					. esc_html__("Судя по всему, вы не настроили никаких сетевых настроек. ", 'ihc') . '<a href="' . admin_url( 'admin.php?page=ihc_manage&tab=payment_settings&subtab=stripe_connect' ) . '">' . esc_html__( 'платежный сервисs', 'ihc' ) . "</a>" . esc_html__(" для вашего членского сайта. Чтобы включить членские выплаты, настройте ", 'ihc') . '<a href="' . admin_url( 'admin.php?page=ihc_manage&tab=payment_settings&subtab=stripe_connect' ) . '">' . esc_html__( "настройки оплаты", 'ihc' ) . "</a> " . esc_html__("", 'ihc') . '</div>';
						...
						if ( ihc_check_payment_available( 'stripe_checkout_v2' ) && empty( $hideSuggestion ) ){
								...
								return '<div class="ihc-error-global-dashboard-message">'
												. '<div class="ihc-close-notice ihc-js-close-admin-dashboard-notice" data-type="payments">x</div>'
												. esc_html__('Чтобы улучшить качество обслуживания клиентов и повысить безопасность транзакций, переключитесь на ', 'ihc')
												. '<a href="' . admin_url( 'admin.php?page=ihc_manage&tab=payment_settings&subtab=stripe_connect' ) . '">'
												. '<strong>Stripe Connect</strong></a> ' . esc_html__('в качестве платежного шлюза с дополнительными функциями.', 'ihc')
												. '</div>';
							...
							&& empty( $hideSuggestion ) ){
								return '<div class="ihc-error-global-dashboard-message">'
											. '<div class="ihc-close-notice ihc-js-close-admin-dashboard-notice" data-type="payments">x</div>'
											. esc_html__( 'Переключиться на', 'ihc' )
											.  '<strong> ' . esc_html__('встроенный платеж', 'ihc') . ' </strong>'
											. esc_html__('форму прямо на вашем сайте с помощью', 'ihc')
											. ' <a href="' . admin_url( 'admin.php?page=ihc_manage&tab=payment_settings&subtab=stripe_connect' ) . '">Stripe Credit Card</a> '
											. esc_html__( 'для повышения коэффициента конверсии и снижения оттока', 'ihc')
											. '</div>';
						}
				}
			}
			...
			if (!ihc_check_payment_available($default_payment)){
				return '<div class="ihc-error-global-dashboard-message"><strong>' .  esc_html__("Платежный сервис по умолчанию либо не активирован, либо настроен неправильно.", 'ihc') . '</strong></div>';
			}
		}
	}
}
...
function ihc_return_membership_plan($level = array(), $currency='' )
{
   ...
   if(isset($level['price'])){
                   if($level['price'] > 0) {
                           ...
                           $price =  esc_html__('Бесплатно','ihc');
                   }
   }
