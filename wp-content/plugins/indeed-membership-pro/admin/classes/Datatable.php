...
class Datatable
{
    public function __construct()
    {
        ...
        add_action('wp_ajax_silesto_update_user_balance_ajax', [ $this, 'silesto_update_user_balance' ]);
    }
          ...
          foreach ( $orders as $order ){
              ...
              if ( $taxesOn ){
                  ...
                  if (isset($firstChage) && $firstChage == true && isset($firstAmount)){
                      ...
                      $array['net_amount'] = ihc_format_price_and_currency( esc_html($order['amount_type']), esc_html($netAmount) );// esc_html($netAmount) . ' ' . esc_html($order['amount_type']);
                  } else {
                      $base_price = $orderMeta->get( $order['id'], 'base_price' );
                      if ( $base_price !== null ){
                        $array['net_amount'] = ihc_format_price_and_currency( esc_html($order['amount_type']), esc_html($base_price) );//esc_html($base_price) . ' ' . esc_html($order['amount_type']);
                      } else if ( $taxes != false ){
                        $netAmount = $order['amount_value'] - $taxes;
                        $array['net_amount'] = ihc_format_price_and_currency( esc_html($order['amount_type']), esc_html($netAmount) );//esc_html($netAmount) . ' ' . esc_html($order['amount_type']);
                      } else {
                        $array['net_amount'] = ihc_format_price_and_currency( esc_html($order['amount_type']), esc_html($order['amount_value']) );//esc_html($order['amount_value']) . ' ' . esc_html($order['amount_type']);
                      }
                  }
                  ...
                  if ( $taxes === false || $taxes === '' ){
                      ...
                      $array['taxes'] = ihc_format_price_and_currency( esc_html($order['amount_type']), $taxes );//$taxes . ' ' . esc_html($order['amount_type']);//
                  }
    ...
    <span class ="iump-btns ihc-delete-link ihc-js-admin-coupons-delete-coupon" data-delete_message="' . esc_html__( 'Are You sure You wish to delete this coupon?', 'ihc' ) . '" data-id="' . esc_attr($coupon['id']) . '" >' . esc_html__('Delete', 'ihc') . '</span>
              ...
              if ($coupon['settings']['discount_type']=='percentage'){
                ...
                $array['discount'] = ihc_format_price_and_currency( esc_html( get_option( 'ihc_currency' ) ), $array['discount'] );//esc_ump_content(' ' ). esc_html( get_option( 'ihc_currency' ) );
              }
              $array['id'] = $coupon['id'];
    ...
    public function saveState()
    {
        ...
        //$state = stripslashes( $state ); // since 12.8.1
        ...
        foreach ( $results as $userData ){
            ...
            */
            ...
            $userRow = [
                          ...
                          'silesto_balance'           => (function($userData) {
                          // Получаем объект пользователя, чтобы проверить его роли
                          $user_object = get_user_by('ID', $userData->ID);

                          // Проверяем, есть ли у пользователя роль 'vip_user'
                          if ($user_object && user_can($user_object, 'vip_user')) {
                              // --- ДА, ЭТО VIP-ПОЛЬЗОВАТЕЛЬ: показываем поле для ввода ---
                              $balance = get_user_meta($userData->ID, 'silesto_user_balance', true);
                              $balance = !empty($balance) ? floatval($balance) : 0;
                              
                              return '<div class="silesto-balance-wrapper">
                                          <input type="number" 
                                                class="silesto-balance-input"
                                                data-uid="' . esc_attr($userData->ID) . '" 
                                                value="' . esc_attr(number_format($balance, 2, '.', '')) . '" 
                                                step="0.01" 
                                                style="width: 100px;" />
                                          <span class="silesto-balance-spinner" style="display: none;"></span>
                                          <span class="silesto-balance-status"></span>
                                      </div>';
                          } else {
                              // --- НЕТ, ЭТО ОБЫЧНЫЙ ПОЛЬЗОВАТЕЛЬ: показываем прочерк ---
                              return '<strong>НЕ ЯВЛЯЕТСЯ VIP</strong>';
                          }
                      })($userData),
                          'wp_role'                     => $roleHtml,
                          'email_status'                => $status,
...
// --- НАЧАЛО ОБНОВЛЕННОГО КОДА СОХРАНЕНИЯ ---

/**
 * AJAX-обработчик для обновления баланса пользователя.
 * (С проверкой на VIP-статус).
 */
public function silesto_update_user_balance()
{
    // Проверяем безопасность и права доступа
    if (!check_ajax_referer('silesto_balance_nonce', 'nonce', false) || !current_user_can('edit_users')) {
    ...
    if (isset($_POST['user_id']) && isset($_POST['balance'])) {
        ...
        if (!$user_object || !user_can($user_object, 'vip_user')) {
             // Если это не VIP-пользователь, прерываем операцию
             wp_send_json_error(['message' => 'Not a VIP user.']);
             return;
        }
        // --- КОНЕЦ ПРОВЕРКИ ---

        $balance = floatval($_POST['balance']);
        update_user_meta($user_id, 'silesto_user_balance', $balance);
        wp_send_json_success(); // Отправляем ответ об успехе
    }

    wp_send_json_error(); // Отправляем ответ об ошибке
}
