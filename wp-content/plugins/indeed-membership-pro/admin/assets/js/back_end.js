		 ...
		 if( jQuery('#ihc_account_page_custom_css').length ) {
			...
			if( jQuery('#ihc_listing_users_custom_css').length ) {
			 ...
			 if( jQuery('#ihc_login_custom_css').length ) {
					 ...
					 /*
					 var cmInstance = iumpCSSEditor.codemirror;

					 // codemirror event for change login custom css
					 cmInstance.on('keyup', function(cm, event) {
						  	var temporaryContent = cm.getValue();
								jQuery('#ihc_login_custom_css').val(temporaryContent);
								ihcLoginPreview();
					 });
					 */
